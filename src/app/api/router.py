...
from app.api.schemas import (
    ...
    WordReplacementRequest,
    WordReplacementResponse,
)
from app.constants import REPLACEMENT_MAP

logger = logging.getLogger("debug-logger")
rt = APIRouter(prefix="/api/word-replacement/v1", tags=["Word replacement"])
...
async def replace_words_from_file_to_stream(file: UploadFile) -> StreamingResponse:
    ...
    return StreamingResponse(
        content=io.BytesIO(processed_file_content.encode()), media_type="text/plain"
    )
